...
const crawlOneBatch = async (supabase: SupabaseClient, env: Env, userId: string): Promise<CrawlResult> => {
  const accessToken = await getValidAccessToken(supabase as unknown as Parameters<typeof getValidAccessToken>[0], userId, env);
  ...
  const needsContent = files.filter((f: { content: string | null; google_drive_id: string | null; file_type: string | null }) =>
    !f.content && f.google_drive_id && f.file_type);
  ...
  await runWithConcurrency(toFetch, CRAWL_CONCURRENCY, async (f: { id: string; file_name: string; file_type: string | null; google_drive_id: string | null }) => {
  ...
  return { crawled, connections: connCreated, indexed: contentMap.size, remaining };
};
